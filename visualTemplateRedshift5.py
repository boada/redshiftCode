...
#tempDir=os.environ['HOME']+os.path.sep+"Astro_Software"+os.path.sep+"VisualTemplateRedshiftTemplates"+os.path.sep
...
class App:
    ...
    def __init__(self, master, objectSpecFileNames, outDir):
        ...
        self.outFile=file(outDir+os.path.sep + \
                datetime.datetime.today().isoformat()+".results", "w")
        self.outFile.write("#Filename\tRedshift\tRedshiftError"+ \
                "\tQuality\tComments\n")
        ...
        self.outPathEntryVar.set(
                self.outDir+os.path.sep+self.objectSpecFileName.replace(
                    ".fits", ".png"))
        self.outPathEntry=Tkinter.Entry(self.buttonFrame,
                textvariable=self.outPathEntryVar, width=80)
        ...
        self.qualityLabel=Tkinter.Label(self.qualityFrame,
                text="Quality flag : ", anchor=Tkinter.E)
        self.qualityLabel.grid(row=2, column=0)
        for i in range(4):
            self.qualityRadioList.append(Tkinter.Radiobutton(self.qualityFrame,
                text = str(i), variable=self.qualityRadioVar, value = i,
        ...
        self.commentsEntry=Tkinter.Entry(self.qualityFrame,
                textvariable=self.commentsEntryVar, width=80)
        ...
        self.smoothLabel=Tkinter.Label(self.smoothFrame,
                text="Spectrum smoothing : ", anchor=Tkinter.E)
        self.smoothLabel.grid(row=1, column=0)
        self.smoothScale = Tkinter.Scale(self.smoothFrame,
            orient=Tkinter.HORIZONTAL, length = 300, from_=0, to=100,
            tickinterval=25, command = self.smoothSpectrum,
            variable=self.smoothScaleVar, resolution=1)
        ...
        self.minWavelengthLabel=Tkinter.Label(self.smoothFrame,
                text="Min WL:", anchor=Tkinter.E)
        self.minWavelengthLabel.grid(row=1, column=6)
        self.minWavelengthEntryVar=Tkinter.StringVar()
        self.minWavelengthEntryVar.set("5000")
        self.minWavelengthEntry=Tkinter.Entry(self.smoothFrame,
        ...
        self.maxWavelengthLabel=Tkinter.Label(self.smoothFrame,
                text="Max WL:", anchor=Tkinter.E)
        self.maxWavelengthLabel.grid(row=1, column=8)
        self.maxWavelengthEntryVar=Tkinter.StringVar()
        self.maxWavelengthEntryVar.set("10000")
        self.maxWavelengthEntry=Tkinter.Entry(self.smoothFrame,
        ...
        self.altNormCheckButton=Tkinter.Checkbutton(self.smoothFrame,
        ...
        self.plotSkyCheckButton=Tkinter.Checkbutton(self.smoothFrame,
                variable=self.plotSkyCheckVar, command=self.plotSkyChanged)
        ...
        for i in range(len(self.templates)):
            try:
                ...
                ipshell()
                sys.exit()
            self.templateRadioList.append(Tkinter.Radiobutton(templatesFrame,
                text = tempName, variable=self.templateRadioVar, value = i,
                command = self.redrawPlot))
        ...
        self.redshiftLabel=Tkinter.Label(scaleFrame,
                text="Template redshift : ", anchor=Tkinter.E)
        self.redshiftLabel.grid(row=3, column=0)
        self.redshiftScale = Tkinter.Scale(scaleFrame,
                orient=Tkinter.HORIZONTAL, length = 600, from_=0.0, to=2.01,
                tickinterval=1, command = self.getRedshiftScaleValue,
                variable=self.redshiftScaleVar, resolution=0.001)
        ...
        for i in range(len(spectralFeaturesCatalogue)):
            ...
            self.featuresCheckLabelsList.append(Tkinter.Label(featuresFrame,
        ...
        self.updatePlot(self.objSED,
                self.templates[self.templateRadioVar.get()], self.skySED,
                self.redshiftScaleVar.get(), tempLabel=\
                os.path.split(templateLabels[self.templateRadioVar.get()])[-1],
    ...
    def getRedshiftScaleValue(self, event):
        ...
        #self.updatePlot(self.objSED, self.templates[self.rbVar.get()],
        #self.skySED, self.scaleVar.get())
    ...
    def runXCSAO(self, templatesToInclude = []):
        ...
        print irafFileName
        result=None
        if os.path.exists(irafFileName) == True or \
            self.convertToIRAFFormat() == True:
            ...
            if fixbad == "y":
                ...
                outFile=file("/opt/iraf_extras/rvsao-2.8.1/lib/badlines.dat",
                        "w")
                for line in lines:
                    outFile.write(str(self.skySED.wavelength[line[0]])+\
                        "\t"+str(self.skySED.wavelength[line[1]])+"\n")
                outFile.close()
            ...
            for temp in templatesToInclude:
                try:
                    rvsao.xcsao(spectra=irafFileName, tempdir=tempDir,
                            fixbad=fixbad, badlines="badlines.dat",
                            vel_init="zguess",
                            czguess=self.redshiftScaleVar.get(),
                            templates=os.path.split(temp)[-1], st_lambda=xMin,
                            end_lambda=xMax, zeropad="y", nsmooth=30,
                            s_emchop="n", t_emchop="n", nzpass=5,
                            minvel=(self.redshiftScaleVar.get()-0.3)*3e5,
                            maxvel=(self.redshiftScaleVar.get()+0.3)*3e5,
    ...
    def next(self):
        ...
        if self.currentSpecFileIndex < len(self.objectSpecFileNames)-1:
            ...
            self.outPathEntryVar.set(self.outDir+os.path.sep+ \
                    self.objectSpecFileName.replace(".fits", ".png"))
            self.redshiftErrorEntryVar.set(0.001)

            self.updatePlot(self.objSED, self.templates[
                self.templateRadioVar.get()], self.skySED,
                    self.redshiftScaleVar.get(),
                    tempLabel=os.path.split(templateLabels[
                        self.templateRadioVar.get()])[-1], redrawSky = True,
...
#                oskyflux=oimg['1D_SPECTRUM'].data.field('SKYSPEC')
    ...
    def updatePlot(self, objSED, tempSED, skySED, redshift, tempLabel = None,
        ...
        if self.altNormCheckVar.get() == 1:
            ...
            binEdges=numpy.arange(objSED.wavelength.min() + ignoreAngstroms,
                    objSED.wavelength.max()-ignoreAngstroms, dw)
            passbands=[]
            for b in binEdges:
        ...
        pylab.text(0.05, 0.92, "z = %.5f $\pm$ %.5f (Q = %s)" % \
                (tempSED.z, float(self.redshiftErrorEntryVar.get()),
                    self.qualityRadioVar.get()), ha='left', va='top',
        ...
        if redrawSky == True and self.plotSkyCheckVar.get() == 1:
            if skySED != None:
                pylab.plot(skySED.wavelength,
                        skySED.flux/skySED.flux.max()*0.3, 'b-', label='Sky')
    ...
    def redrawPlot(self):
        ...
        self.updatePlot(self.objSED, self.templates[
            ...
            self.redshiftScaleVar.get(),
            tempLabel=os.path.split(templateLabels[
                self.templateRadioVar.get()])[-1], redrawFeatures=True,
                plotFeatures=self.plotFeatures)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        ...
        root = Tkinter.Tk()
        root.title("Visual Template Redshift 5.0")
